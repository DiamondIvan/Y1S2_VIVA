...
public class Main {
    ...
    // (Undo is handled by CartList.undo().)
    ...
    private static void handleCheckout(CartList cart, LinkedListStack<CartAction> undoStack, InventoryManager inventory,
        ...
        // Stock has already been reduced when items were added; checkout makes it
        // final.
    ...
    private static String resolveInventoryFile() {
        File f1 = new File("Assignment2" + File.separator + "inventory.txt");
        if (f1.exists()) {
            return f1.getPath();
        }
